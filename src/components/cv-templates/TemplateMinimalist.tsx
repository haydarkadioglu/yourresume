...
export function TemplateMinimalist({ data }: { data: ResumeData }) {
  const contactInfo = [
    ...
    data.personalInfo.linkedin && <a href={`https://linkedin.com/in/${data.personalInfo.linkedin}`} target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">{`linkedin.com/in/${data.personalInfo.linkedin}`}</a>,
